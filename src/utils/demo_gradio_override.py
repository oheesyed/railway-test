...
def demo_gradio_override(self, height=450, dark_mode=True, **kwargs):
    ...
    with gr.Blocks(js=js, theme=gr.themes.Soft()) as demo:
        gr.Markdown("# Personal Assistant Agency")
        ...
        def check_auth(token):
            if auth(token):
                return {chat_interface: gr.Column(visible=True)}
            else:
                return {chat_interface: gr.Column(visible=False)}
        
        auth_button.click(check_auth, inputs=[token_input], outputs=[chat_interface])
